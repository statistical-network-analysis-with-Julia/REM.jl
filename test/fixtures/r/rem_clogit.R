...
# REM.jl's `se=:sandwich` is the event-clustered Godambe sandwich H^-1 B H^-1
# with B the outer product of the per-STRATUM score contributions. In R that is
# coxph's robust variance with the stratum as the cluster:
# `clogit(... + strata(stratum) + cluster(stratum))`. Two details matter:
#   * `cluster(stratum)` is required. Without it coxph's robust variance is the
#     per-ROW dfbeta sandwich, a different number (1.2e-3 apart on this design).
#   * clogit refuses a robust variance under its default `method = "exact"`;
#     with exactly one case per stratum the Breslow approximation IS the exact
#     conditional likelihood, so `method = "breslow"` gives the same MLE and
#     the robust variance we want (coefficients identical to the fit above).
fit_robust <- clogit(is_event ~ repetition + reciprocity + sender_activity +
                       receiver_popularity + transitive_closure +
                       strata(stratum) + cluster(stratum),
                     data = df, method = "breslow",
                     control = coxph.control(eps = 1e-11, toler.chol = 1e-14,
                                             iter.max = 200))
stopifnot(max(abs(coef(fit_robust) - coef(fit))) < 1e-9)
robust_se <- sqrt(diag(vcov(fit_robust)))    # vcov() of a cluster fit is robust
...
cat("default = 1e-8\n")
cat("# The robust (cluster = stratum) sandwich is the same closed-form H^-1 B H^-1\n")
cat("# on both sides, evaluated at MLEs that agree to < 1e-13: observed\n")
cat("# disagreement 4e-16, so 1e-8 again leaves eight orders of margin.\n")
cat("robust_std_errors = 1e-8\n")
...
cat("# survival::clogit(... + strata(stratum) + cluster(stratum), method = \"breslow\"):\n")
cat("# the stratum-clustered robust variance, REM.jl's se=:sandwich\n")
cat(sprintf("robust_std_errors = [%s]\n", num(robust_se)))
